...
function Serialize-UIDefinition {
    ...
    try {
        ...
        foreach ($step in $Definition.Steps | Sort-Object Order) {
            ...
            foreach ($control in $step.Controls) {
                ...
                if ($control.Type -in $cardTypes) {
                    ...
                    $cardJson = @{
                        ...
                        Title = $control.Label
                        Subtitle = $control.GetPropertyOrDefault('CardDescription', $null)
                        Properties = @{}
                    }
                    
                    # Copy all properties from control
                    foreach ($key in $control.Properties.Keys) {
                        ...
                        if ($value -is [scriptblock]) {
                            ...
                            $cardJson.Properties[$key] = $scriptContent
                        }
                        else {
                            $cardJson.Properties[$key] = $value
                        }
                    }
                    
                    # Add common card properties
                    if ($control.GetProperty('Icon')) {
                        $cardJson.Properties['Icon'] = $control.GetProperty('Icon')
                    }
                    if ($control.GetProperty('Category')) {
                        $cardJson.Properties['Category'] = $control.GetProperty('Category')
                    }
                ...
                elseif ($control.Type -eq 'Banner') {
                    ...
                    if ($dsScriptBlock) {
                        $controlJson.DataSourceScriptBlock = $dsScriptBlock
                    }
            ...
            $stepJson.Controls = $regularControls
            $stepJson.Cards = $cards
            if ($banner) {
        ...
        $jsonObject = @{
            ...
            Template = "Dashboard"  # HARDCODED - Dashboard module always uses Dashboard template
